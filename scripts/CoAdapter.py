...
def url_to_numpy(url: str):
    response = requests.get(url)
    response.raise_for_status()  # 예외 발생 시 중단

    image_data = BytesIO(response.content)  # 이미지 데이터를 메모리 버퍼에 저장
    image = Image.open(image_data)
    image_array = np.array(image)

    return image_array

def convert_base64_in_list(inps):
    out = []
    for item in inps:
        if isinstance(item, tuple):
            converted = tuple(url_to_numpy(i) if isinstance(i, str) and i.startswith("http") else i for i in item)
...
def is_base64(s: str) -> bool:
    ...
    # base64 문자열은 길이가 4의 배수여야 합니다.
    if len(s) % 4 != 0:
        return False

    # base64 문자열은 영문 대소문자, 숫자, '+', '/', '='로만 이루어져야 합니>다.
